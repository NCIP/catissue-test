...
########################################################################################
# Funcation Name: Read_ppm_conf_file
# Description   : Reads Configuration paramters from config file
# Arguments		: Config file name
# Return Values : caTissue.Home,read_ppm_conf_file, Summary.Report, Detail.Report
#########################################################################################

sub read_ppm_conf_file {
    my $file_name = $_[0];
    my @file_data;
    my $line;
    ...
    foreach $line (@file_data) {
		...
		elsif ( $line =~ /^(Input.Files=)/g ) {
            $val_6 = $';
        }
		elsif ( $line =~ /^(Drop.SQL.Files=)/g ) {
            $val_7 = $';
        }
		elsif ( $line =~ /^(Output.Files=)/g ) {
            $val_8 = $';
        }
		elsif ( $line =~ /^(Summary.Report=)/g ) {
            $val_9 = $';
...
sub read_catissue_prop_file {
    ...
    if (   $database_type eq ""
        ...
        || $database_port     eq ""
        || $database_name     eq ""
        || $database_username eq ""
        || $database_password eq ""
        || $jboss_server_host eq ""
        || $jboss_server_port eq "" )
    {
...
#####################################################################################################
# Funcation Name: Read_PPM_prop_file
# Description   : Reads privatePublic.property file to get private ans public database credentials	
# Arguments     : Property file name & Application name
# Return Values : $databaseType, $privateDBName, $privateDBHost, $privateDBUserName, $privateDBPassword,
...
sub read_ppm_prop_file {
    ...
    foreach $line (@file_data) {
        ...
        if ( $line =~ /^(databaseType=)/g ) {
            $databaseType = $';
        }
        elsif ( $line =~ /^(privateDBName=)/g ) {
            $privateDBName = $';
        }
        elsif ( $line =~ /^(privateDBHost=)/g ) {
            $privateDBHost = $';
        }
        elsif ( $line =~ /^(privateDBUserName=)/g ) {
            $privateDBUserName = $';
        }
        elsif ( $line =~ /^(privateDBPassword=)/g ) {
            $privateDBPassword = $';
        }
        elsif ( $line =~ /^(privateDBPort=)/g ) {
            $privateDBPort = $';
        }
        elsif ( $line =~ /^(privateDBTablespace=)/g ) {
            $privateDBTablespace = $';
        }
        elsif ( $line =~ /^(privateDBIndexTablespace=)/g ) {
            $privateDBIndexTablespace = $';
        }
        elsif ( $line =~ /^(publicDBName=)/g ) {
            $publicDBName = $';
        }
        elsif ( $line =~ /^(publicDBHost=)/g ) {
            $publicDBHost = $';
        }
        elsif ( $line =~ /^(publicDBUserName=)/g ) {
            $publicDBUserName = $';
        }
        elsif ( $line =~ /^(publicDBPassword=)/g ) {
            $publicDBPassword = $';
        }
	 elsif ( $line =~ /^(publicDBPort=)/g ) {
            $publicDBPort = $';
        }
	 elsif ( $line =~ /^(publicSystemUserName=)/g ) {
            $publicSystemUserName = $';
        }
	 elsif ( $line =~ /^(publicSystemPassword=)/g ) {
            $publicSystemPassword = $';
        }
	 elsif ( $line =~ /^(publicDBTNSName=)/g ) {
            $publicDBTNSName = $';
        }
    }
    ...
    if (   $databaseType eq ""
        || $privateDBName     eq ""
        || $privateDBHost     eq ""
        || $privateDBUserName     eq ""
        || $privateDBPassword eq ""
        || $privateDBPort eq ""
        || $privateDBTablespace eq ""
        || $privateDBIndexTablespace eq ""
		|| $publicDBName eq ""
		|| $publicDBHost eq ""
		|| $publicDBUserName eq ""
		|| $publicDBPassword eq ""
		|| $publicDBPort eq ""
		|| $publicSystemUserName eq ""
		|| $publicSystemPassword eq ""
		|| $publicDBTNSName eq ""		)
    {
        die("\nCould not read necessary private public properties.\n");
    }
...
########################################################################################
# Funcation Name : write_summry_report_ppm
# Description    :
# Arguments      : $summary_report,$total_tc,$tc_pass,$tc_fail,$tc_aborted
# Return Values  :  -
#########################################################################################

sub write_summry_report_ppm {
    my $file_name = shift;    #it will assign 1st argument
    my $csv_row =
      join( ',', @_ );    # it will create single row of coma seperated values
    open( FP, ">>$file_name" );
...
########################################################################################
# Funcation Name : generate_html_report_ppm
# Description    :
# Arguments      : $html_report,$summary_report,$detail_report
# Return Values  : -
#########################################################################################

sub generate_html_report_ppm {
    my $html_file    = $_[0];
    my $summary_file = $_[1];
    my $detail_file  = $_[2];
    my ( $line, @a, $header );
...
# Funcation Name	: cleaning_old_artifacts
# Description		:
# Arguments			: detail_report, summary_report
# Return Values		: -
########################################################################################

sub cleaning_old_artifacts_ppm {
...
sub initial_setup_ppm {
	...
	write_summry_report( $_[1], "Total Test cases",
